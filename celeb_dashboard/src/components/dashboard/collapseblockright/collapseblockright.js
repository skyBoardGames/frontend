...
const Block = ({ 
  handleclose, goToDeposit, goToGames, openWithdrawModal, pathname, joinLobbyModal, withdrawModal,
  isOpen
}) => {
  return (
    <>
      <div className='collapseback py-3 d-lg-block d-md-block d-none' onClick={handleclose} disabled={!isOpen}>
        Collapse Block <CustomSvg name="arrow-right" />
      </div>
        ...
        </div>
        {
          !pathname.includes('deposit') && !pathname.includes('join-lobby') && !pathname.includes('selected-game') && !pathname.includes('games') && !pathname.includes('tournaments/play') &&
            <>
              <button onClick={goToGames} className='py-3 createlobby px-3 w-100'>Create Lobby <CustomSvg name="arrow-right" color="black" /></button>
              <button className='py-3 joinlobby px-3 w-100'>Join Lobby <CustomSvg name="arrow-right" /></button>            
            </>
        }
  ...
  return (
    <div className=''>
      {(showOpenButton || isSmallScreen) && ( 
        <div>
          <button className='d-lg-block d-md-block d-none arrowLeft mb-lg-0 mb-md-0 mb-4' onClick={handleOpen}><CustomSvg name="arrow-left" /></button>   
          <button className='d-lg-none d-md-none d-block arrowLeft mb-lg-0 mb-md-0 mb-4' onClick={openOffCanvasBlock}><CustomSvg name="arrow-left" /></button>   
        </div>
      )}
      {isOpen && (
