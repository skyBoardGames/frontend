...
function WatingRoom() {
  ...
  const goToReRoutePath = ({ reRoutePath }) => navigateTo(reRoutePath)
  ...
  useEffect(() => {
    const waitingTimeout = setTimeout(() => {
      if(!location.state){
        ...
        goToReRoutePath({ reRoutePath: location.state.reRoutePath })
      }
      ...
      <div className='p-3 d-lg-flex d-md-flex d-block align-items-start justify-content-between'>
          ...
          <div className={`${blocksOpen == 'both' ? 'col-lg-8 col-md-8' : blocksOpen == 'one' ? 'col-lg-9 col-md-9' : blocksOpen == 'none' ? 'col-lg-10 col-md-10' : ''} col-12 px-lg-4 px-md-4 px-0 d-flex flex-column align-items-center`}>
              <div className='w-100'>
                {
                  gameId != null &&
                    <img className='col-lg-12 col-md-12 col-12' src={allGames.filter(game => game.id == gameId)[0].img} />
                }
              </div>
