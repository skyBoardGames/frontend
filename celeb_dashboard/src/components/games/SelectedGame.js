...
export default function SelectedGame(){
    ...
    const goToTournaments = (data) => navigateTo(`/tournaments/play/${gameId}`, data)
    ...
    const [hostGameModal, setHostGameModal] = useState({ visible: false, onHide: null, size: 'md' })
    const [gateFeeModal, setGateFeeModal] = useState({ visible: false, onHide: null, size: 'md' })
    ...
    const [winnersPrices, setWinnersPrices] = useState([{ price: null, position: 1 }])
    ...
    const openHostGameModal = () => setHostGameModal({ visible: true, onHide: hideHostGameModal, size: 'md' })
    const hideHostGameModal = () => setHostGameModal({ visible: false, onHide: null, size: 'md' })

    const openGateFeeModal = ({ numWinners }) => {
        setGateFeeModal({ visible: true, onHide: hideGateFeeModal, size: 'md' })
        setWinnersCount(numWinners)
    }
    const hideGateFeeModal = () => setGateFeeModal({ visible: false, onHide: null, size: 'md' })
    ...
    if(gameId){
        ...
        const selectedGame = allGames.filter(game => game.id == gameId)
        const { id, bgClass, img, title, caption, text, splitTitle1, splitTitle2, maxPlayers } = selectedGame[0]

        const displayWinnersPrices = Array.from(Array(winnersCount).keys()).map((el, i) => {
            ...
            const btnFunc = (price) => {
                ...
                if(positionPresent){
                    ...
                    if(winnersCount == updatedWinnersPrices.length){
                        ...
                        setWinnersPrices([...updatedWinnersPrices, { price: null, position: winnerPosition+1 }])
                    }
            ...
            return (
                <div
                    ...
                    className='mb-4'
                    style={{
                        display: winnersPrices.length == winnerPosition ? 'block' : 'none'
                    }}
                >
                    <h1 className='m-0 p-0 mb-3 font-weight-700 font-family-quantico txt-large txt-FFF'>Set <span className='txt-73CD02'>{winnerPosition}-Place Price</span></h1>
                        ...
                        <div>
                            <div className='d-flex align-items-center justify-content-stretch'>
                                <div className='w-100'>
                                    ...
                                    {
                                        winnersCount == 0 &&                                        
                                            <h1 className='m-0 p-0 mb-3 font-weight-700 font-family-quantico txt-large txt-FFF'>{splitTitle1} <span className='txt-73CD02'>{splitTitle2}</span></h1>
                                    } 

                                    {
                                        showBtns &&
                                            <div>
                                                ...
                                                >
                                                    <p className='p-0 m-0 small-txt txt-000 font-weight-500 font-family-poppins mx-1'>Host Game</p>
...

                                            {
                                                ...
                                                ?
                                                    <div
                                                        className='mb-4'
                                                    >
                                                        <h1 className='m-0 p-0 mb-3 font-weight-700 font-family-quantico txt-large txt-FFF'>Gate <span className='txt-73CD02'>Fee</span></h1>
                                                        <p className='m-0 p-0 mb-4 regular-txt font-weight-300 font-family-poppins txt-FFF'>
                                                            Select Gate Fee
                                                        </p>
                                                        <AmountPicker 
