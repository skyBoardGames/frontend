...
export default function SelectedGame(){
    ...
    const goToTournaments = ({ user_id, stakeValue }) => navigateTo(`/tournaments/play/${user_id}/${gameId}/${stakeValue}`)
    ...
    const onSelectUser = (user) => goToTournaments({ user_id: user.user_id, stakeValue  })

    if(gameId){

        const selectedGame = allGames.filter(game => game.id == gameId)
        const { id, bgClass, img, title, caption, text, splitTitle1, splitTitle2 } = selectedGame[0]
                                ...
                                showGames &&
                                    <div className='d-flex align-items-center justify-content-stretch'>
                                        <div className='w-100'>
                                            ...
                                            <h1 className='m-0 p-0 mb-3 font-weight-700 font-family-quantico txt-large txt-FFF'>{splitTitle1} <span className='create-lobby-title-span'>{splitTitle2}</span></h1>
    
                                            {
                                                showBtns &&
                                                    <div>
                                                        ...
                                                        >
                                                            <p className='p-0 m-0 small-txt txt-000 font-weight-500 font-family-poppins mx-1'>Create Lobby</p>
